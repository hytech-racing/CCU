...
void ACUInterface::receive_status_message(const CAN_message_t &msg, unsigned long curr_millis) {
    ...
    _curr_data.acu_state = bms_status_msg.charging_state;
...
void ACUInterface::receive_current_temps_message(const CAN_message_t& msg, unsigned long curr_millis)
{
    ...
    group_start_index = chip_id * _acu_params.temp_cell_groups_per_ic * _acu_params.temp_cells_per_group + group_id * _acu_params.temp_cells_per_group;
...
void ACUInterface::receive_onboard_current_temps(const CAN_message_t& msg, unsigned long curr_millis)
{
...
void ACUInterface::receive_cell_voltages(const CAN_message_t& msg, unsigned long curr_millis)
{
    ...
    if (group_id % 2 == 0)
        group_start_index = 
        (_acu_params.voltage_cell_groups_per_ic_even + 
            ((chip_id+1) / 2) * 
            (_acu_params.voltage_cell_groups_per_ic_even + _acu_params.voltage_cell_groups_per_ic_odd)) * 
            _acu_params.voltage_cells_per_group + 
            group_id * _acu_params.voltage_cells_per_group;
    else
        group_start_index = ((chip_id+1) / 2) * (_acu_params.voltage_cell_groups_per_ic_even + _acu_params.voltage_cell_groups_per_ic_odd) * _acu_params.voltage_cells_per_group + group_id * _acu_params.voltage_cells_per_group;
