...
{
    for (uint32_t i = 0; i < msg_in.voltages_count; ++i) //protoc struct is an array of floats
    { 
        acu_all_data.voltages[i] = msg_in.voltages[i]; //should be passed in as normal voltage based on vcr
    }
